...
},{"../..":3,"./dip-8.json":1}],3:[function(require,module,exports){
...
},{"./lib/bootstrap.js":5}],4:[function(require,module,exports){
// pinboardjs
// The background draws shapes upon which pins are placed.
// 
// (c) patrick mulder, 2014

var _ = require('underscore');

Background = function(svg, options) {

  this.svg = svg;
  this.skin_background = options.skin.background; 

  this.initialize.apply(this, arguments);
};

Background.prototype.initialize = function(s, options) {

  // derive basic layout parameters from skin
  this.outer_height = this.skin_background.outer.height;
  this.outer_width = this.skin_background.outer.inner_width + this.verticalWidth;
  this.inner_height = this.skin_background.inner.height;
  this.inner_width = this.skin_background.inner.width;

  this.color = this.skin_background.color;
}
...
Background.prototype.render = function() {
  ...
  var west = this.svg.rect(xOffset, yOffset + this.northHeight, this.verticalWidth, this.verticalHeight);
  ...
  var south = this.svg.rect(xOffset, yOffset + this.outerHeight, this.outerWidth, this.northHeight);
...
module.exports = function(domNode, designData, modules) {

  var svg = new Svg(domNode);

  var grid = new Grid(svg);
  grid.render();

  var design = new Design(svg, designData, modules);
  design.render();
}

},{"./background":4,"./design":6,"./grid":7,"./s":9,"underscore":12}],6:[function(require,module,exports){
// pinboardjs
// The design places pins according to skin and
...
Design = function(s, data, options) {

  // reference to canvas for drawing
  this.svg = s;
  this.title = data.title;
  this.components = data.components;
  this.modules = _.has(options, 'modules') ? options.modules : [];

};

// to override with custom initialization 
Design.prototype.initialize = function() {}

Design.prototype._renderComponent = function(c) {

  var w = c.Xend - c.Xstart + 2 * c.width;
  var h = c.Yend - c.Ystart + 2 * c.width;

  r = this.svg.rect(c.Xstart, c.Ystart, w, h);
  r.attr({fill: '#000000', stroke: '#000000'});

  r = this.svg.rect(c.Xstart + c.width, c.Ystart + c.width, w - 2*c.width, h - 2*c.width);
  r.attr({fill: c.color, stroke: '#000000'});
...
Design.prototype._renderModule = function(c) {

  var designModule = _.find(this.modules, {'name': c.type})
  this._renderComponents(designModule.components, this);

}

Design.prototype._renderDrawing = function(drawings) {

  if (drawings.type == 'segment') {
    this._renderComponent(drawings);
  } else {
    this._renderModule(drawings);
  }

}

Design.prototype._renderComponents = function(components, ctx) {

  components.forEach(function(c) {
    if (_.has(c, 'drawings')) {
      var drawings = c.drawings;

      if (_.isArray(drawings)) {
        drawings.forEach(function(d) {
          ctx._renderDrawing(d);
        })
      } else {
        ctx._renderDrawing(drawings);
      }
    }
  });
...
Design.prototype.render = function() {

  this._renderComponents(this.components, this);
  
}

module.exports = Design;

},{"./layouter.js":8,"underscore":12}],7:[function(require,module,exports){
var _ = require('underscore');
...
// pinboardjs
// s.js wraps SnapSVG and a reference to the '#main' DOM node
// 
// (c) patrick mulder, 2014
...
module.exports = function(domElement) {
  return new Snap(domElement);
};

},{"snapsvg":10}],10:[function(require,module,exports){
// Snap.svg 0.2.0
// 
